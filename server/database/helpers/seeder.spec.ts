...
describe('#runDefault()', () => {
  ...
  test('should apply correct default seeders in supplied order', async () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      expect(Logger.warn).toHaveBeenCalledTimes(0);
      expect(Logger.info).toHaveBeenCalledTimes(1);
      const mockedRandomDefaultSeeders = JSON.parse(randomDefaultFile).seeders;
...
describe('#applySeeder()', () => {
  ...
  test('should run seeder file when specified with extension', async () => {
    ...
    const mockSeederFilepath = path.join(__dirname, mockSeederDir, mockSeeder);
  ...
  test('should assume .ts when seeder specified with no extension', async () => {
    ...
    // should assume .ts
    const mockSeederFilepath =
      path.join(__dirname, mockSeederDir, mockSeeder) + '.ts';
    expect(runSeederSpy).toHaveBeenCalledTimes(1);
    expect(runSeederSpy).toHaveBeenCalledWith(mockSeederFilepath, mockMongo);
  });
...
const generateRandomMockDefaultFile = (size?: number): string => {
  if (!Number.isInteger(size)) {
    size = Math.trunc(Math.random() * 100);
  }

  let mockDefaultFile = '{"seeders": [';
  if (size > 0) {
    mockDefaultFile += '"hello1"';
  }
  for (let i = 1; i < size; i++) {
    mockDefaultFile += `, "hello-${i}"`;
  }
  mockDefaultFile += ']}';
  return mockDefaultFile;
};
